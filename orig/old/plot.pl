...
my %default_opts = (
		    ...
		    errb => 1,
		    datadir => '../data',
		    hrcfitdir => '../fits/qe_N0014_qeu_N0009',
		    acisfitdir => '../fits/acis',
		    );
my %opts = %default_opts;
GetOptions(\%opts,
	   'help!', 'version!', 'debug!',
	   'binned!', 'errb!', 'datadir=s', 'hrcfitdir', 'acisfitdir',
	   'dev=s', 'rdb=s',
...
sub read_fit_params {
  ...
  for my $obsid (@$obsids) {
    ...
    my @globs = $opts{binned} ? ($opts{datadir}."/$src/$obsid/tg_reprocess/evt2_??.fits")
      : ($opts{datadir}."/$src/$obsid/tg_reprocess/*_evt2.fits");
    ...
    for my $evt2 (@files) {
      ...
      if ($h->{DETNAM} =~ /hrc/i) {
	@k = @hrc;
	$fitdir = $opts{hrcfitdir}
      }

      elsif ($h->{DETNAM} =~ /acis/i) {
	$fitdir = $opts{acisfitdir};
	...
	for my $o (qw/ neg pos /) {
	  ...
	  my $f =
	    $k =~ /xs/ ? "$dir/${obsid}_xsLEG_${o}_B${band}.log" :
	      $k eq 'hrc' ? "$dir/${obsid}_LEG_${o}_B${band}" :
		$dir . "/${obsid}_" .uc($k). "_${o}_B${band}";

	  if ($opts{binned}) {
	    my ($n) = $evt2 =~ /(_\d{2})\.fits/ or die $evt2;
	    $f =~ s/_B${band}/_B${band}$n/;
	  }
...
#    $obsid eq '11974' and next;
    ...
    my ($pha2) = glob("$opts{datadir}/$src/$obsid/tg_reprocess/*_pha2.fits") or die;
    my $h = Astro::FITS::CFITSIO::fits_read_header($pha2.'[spectrum]');
    my $tstart = $h->{TSTART};
