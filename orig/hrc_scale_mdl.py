...
def plot(data, sim_times, scale, orders, polys, args):
    ...
    for det in data:
        start[det], stop[det] = hrc_scale.get_gaps(data[det]['time'])

    pdf = PdfPages('mdl_%s_B%d_E%d_poly_%02d.pdf' % (source, band, epoch, orders[0]))
    fig = plt.figure(figsize = (11, 8.5))

    if epoch == 0:
        plot_dims = (2, int(math.ceil(start['acis'].size/2.)))
    else:
        plot_dims = (1, 1)
    ...
    for i in range(scale.size):
        ...
        for j in range(start['acis'].size):
            ...
            if col==0: plt.ylabel('%g - %g keV flux' % hrc_scale.band_range(band))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Perform CARMA simulation models for combined ACIS and HRC-S fluxes, calculate length of model paths while scaling HRC-S flux.'
    )
    ...
    for det in 'leg', 'hrc':
        ...
        try:
            data[det] = hrc_scale.readflux( rdbfile )
        except IOError:
    ...
    if epoch > 0:
        for det in data:
            start, stop = hrc_scale.get_gaps(data[det]['time'])
            if epoch > start.size:
    ...
    mdl = np.zeros_like(scale)

    time = np.concatenate([data[det]['time'] for det in 'acis', 'hrc'])
    n = time.size
    ...
    for j in xrange(orders.size):
        ...
        for i in xrange(scale.size):
            flux = np.concatenate( (data['acis']['flux'], data['hrc']['flux'] * scale[i]) )
            err = np.concatenate(  (data['acis']['err'],  data['hrc']['err'] * scale[i] ) )
        ...
        min_mdl, min_scale = mdl.min(), scale[np.argmin(mdl)]
    ...
    if args.models:
        global ntimes
        sim_times = hrc_scale.get_sim_times(data, ntimes)
        plot(data, sim_times, scale, orders, polys, args)
